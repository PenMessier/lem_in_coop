...
/*   Updated: 2020/05/10 11:25:46 by Elena            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (*input && !ft_strcmp(input, "##start"))
	{
		*nl = 0;
		return (!map->start ? 1 : -2);
	}
	else if (*input && !ft_strcmp(input, "##end"))
	{
		*nl = 1;
		return (!map->end ? 1 : -3);
	}
...
int			ft_parse(char *input, t_map *map, int *nl)
{
	...
	if (*nl != 0 && *nl != 1 && (f = check_comands(input, map, nl)))
		return (f);
	if (*nl == 0 || *nl == 1 || *nl == 3)
